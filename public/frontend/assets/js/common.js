...
(function ($) {
	...
	function twitterWidget() {
		...
		if (footerTwitterFeedVertical !== null) {
			var configTwitterVertical = {
				'profile': {
					'screenName': footerTwitterFeedVertical.getAttribute('data-twitter-id')
				}, // <- your twitter ID frim data-attribute here
				'id': footerTwitterFeedVertical.getAttribute('data-twitter-id'), // deprecated
	...
	function onePage() {
		...
		$porfolioCategories.each(function () {
			var category = this.getAttribute('data-category').toLowerCase().trim().split(/\s*,\s*/);

			for (var i = 0; i < category.length; i++) {
				$(this).addClass('portfolio-filter-' + category[i].replace(/\s/g, '-'));

				if (!porfolioCategories[category[i]])
					porfolioCategories[category[i]] = category[i];
			}
		});
		...
		for (var porfolioCategory in porfolioCategories) {
			if (porfolioCategories.hasOwnProperty(porfolioCategory)) {
				filterItems += '<span class="portfolio-filter__item" data-filter=".portfolio-filter-' + porfolioCategory.replace(/\s/g, '-') + '">' + porfolioCategory + '</span>';
			}
		};
		$(filterElem).append(filterItems);
